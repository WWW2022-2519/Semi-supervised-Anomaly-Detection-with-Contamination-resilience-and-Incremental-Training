...
In the inference stage, training data are used to learn posterior probabilities $P(X \mid y)$. In the decision stage, these posterior probabilities are employed to estimate class conditional probability $P(y\mid X)$. In practice, $https://latex.codecogs.com/gif.latex?P(X \mid y)$ is often used to substitute $P(y\mid X)$. **Due to the low base-rate of data in a certain class, $P(X \mid y)$ calculated from training data is heavily unequal to the class conditional probability $P(y\mid X)$, hence resulting in poor classification performance**. And this problem is termed base-rate fallacy.
